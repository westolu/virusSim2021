...
public class Grapher implements Runnable{
    ...
    private void initialize() throws Exception{
        ...
        for(int i = 0; i<prefs.vars[0]; i++){
            xPos[i] = ThreadLocalRandom.current().nextInt(1, prefs.vars[1] - ovalWidth);
            yPos[i] = ThreadLocalRandom.current().nextInt(1, prefs.vars[2] - ovalHeight);
    ...
    public void tickRun(){
        try {
            for(int i = 0; i < prefs.vars[0]; i++){
                ...
                if(xPos[i] >= prefs.vars[1] - ovalWidth){
                    xVel[i] = xVel[i] * -1;
                }
                if(yPos[i] >= prefs.vars[2] - ovalHeight){
                    yVel[i] = yVel[i] * -1;
                }
                if(xPos[i] <= 0){
                    xVel[i] = xVel[i] * -1;
                ...
                if(xPos[i] + ovalWidth/2 == xPos[j] && yPos[i] + ovalHeight/2 == yPos[j]){
                    infected[i] = true;
                }
            }
            }
